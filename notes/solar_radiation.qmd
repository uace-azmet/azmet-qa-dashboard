...
hourly_sol <-
  ...
  #TODO expand grid to include 00:00, 00:15, 00:30, 00:45, and 00:59:59
  ...
  mutate(
    ...
    solar_rad = sol_rad_total(
      ...
      tau = 0.6, #atmospheric transmissivity, not sure how to estimate
      # elev = elev,
      elev = 1000,
      rho = 1 #albedo, not sure how to estimate, but probably high??
    ) |> set_units("W m-2"),
    solar_rad_direct = direct_solar_radiation(
      lat = latitude,
      doy = doy,
      elev = 1000,
      t = hour,
      ...
      method = "Gates 1962" #This method returns some NaNs that probably should be coerced to zeros?
    ) 
  ) |> 
  mutate(solar_rad_direct = ifelse(is.nan(solar_rad_direct), 0, solar_rad_direct)) |> 
  mutate(solar_rad_direct = set_units(solar_rad_direct, "W m-2"))

hourly_df <-
  left_join(
    hourly,
    hourly_sol |> select(meta_station_id, date_datetime, solar_rad_direct, hour)
  ) |>
  mutate(sol_rad_total = (sol_rad_total / set_units(1, "hr")) |> set_units("W m-2")) 

hourly_df |>
  filter(sol_rad_total > solar_rad_direct) |>
  select(meta_station_id, date_datetime, sol_rad_total, solar_rad_direct, hour) |> 
  # group_by(meta_station_id) |> 
  count(hour)

#only messed up in mornings and evenings.  whyyyyy
  
hourly_df |> 
  filter(meta_station_id %in% c("az01", "az05", "az32")) |> 
ggplot(aes(x = date_datetime)) +
  geom_line(aes(y = sol_rad_total)) +
  geom_line(aes(y = solar_rad_direct), color = "red") +
  facet_wrap(~meta_station_id)
...
-   What exactly is being reported in the hourly data?
    An instantaneous measurement at the end of the hour, beginning of hour, middle of hour?
    An average over the past hour?
    Does this help explain the apparent "phase shift".
    According to metadata it's an hourly total, but how do I convert the instantaneous (?) estimates in W/m^2^ to the hourly total in MJ/m^2^ ?
    Is multiplying by 1 hr correct?
